...
template <typename dataType> class binNode 
{
   ...
   public:
      ...
      // copy constructor
      binNode(const binNode<dataType> &other) : 
      ...
      bool rebalance(binNode<dataType>* &root) 
      {
         ...
         if (rnodes > lnodes) 
         {
            ...
            if (right != NULL && right->right != NULL) 
               rnodes = right->right->numNodes(); 
            nodes = 0;
            if (right != NULL && right->left != NULL) 
               nodes = right->left->numNodes();
         ...
         else if (lnodes > rnodes) 
         {
            ...
            if (left != NULL && left->left != NULL) 
               lnodes = left->left->numNodes();
            nodes = 0;
            if (left != NULL && left->right != NULL) 
                nodes = left->right->numNodes();
      ...
      double balance() const 
      {
         ...
         if (dif <= 1) return bal;
         else return bal / log((double)dif);
      }    
